...
class BrokerTransport:
    """Small synchronous HTTP transport used by the sync client API."""
...
class AsyncBrokerTransport:
    """Small asynchronous HTTP transport used by the async client API."""
    ...
    async def post(self, path: str, payload: dict[str, Any]) -> TransportResponse:
        try:
            async with httpx.AsyncClient(timeout=self.config.timeout) as client:
                r = await client.post(self.config.url.rstrip("/") + path, json=payload)
                r.raise_for_status()
                self.local_mode = False
                return TransportResponse(True, r.json(), False)
        except Exception as e:
